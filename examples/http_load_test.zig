...
fn onHttpSocketWritable(_: Allocator, s: *zs.Socket) !*zs.Socket {
    ...
    http_socket.offset += @intCast(try s.write(request[http_socket.offset..], false));
    return s;
}
...
fn onHttpSocketData(_: Allocator, s: *zs.Socket, _: []u8) !*zs.Socket {
    const http_socket = s.ext(HttpSocket).?;
    http_socket.offset = @intCast(try s.write(request, false));
    responses += 1;
    return s;
}
...
fn onHttpSocketOpen(allocator: Allocator, s: *zs.Socket, _: bool, _: []u8) !*zs.Socket {
    ...
    connections -= 1;
    if (connections != 0) {
        _ = try s.ctx().connect(allocator, HttpSocket, host, port, null, 0);
    } else {
...
pub fn main() !void {
    ...
    _ = try http_context.connect(allocator, HttpSocket, host, port, null, 0);
